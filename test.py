...
def load_lm3d():
    pth = '/content/face_de_mask/BFM/similarity_Lm3D_all.mat'
    Lm3D = loadmat(pth)
    Lm3D = Lm3D['lm']
...
class Test():
    def __init__(self):
        device = 'cuda:0'
        self.img_root = '/content/face_de_mask/input'
        self.ldmk_root = '/content/face_de_mask/ldmk'
        ...
        state_dict = torch.load('/content/face_de_mask/ckpt/3D_it_500000.pkl')
        self.face_encoder.load_state_dict(state_dict)
        self.face_encoder.eval()
        state_dict = torch.load('/content/face_de_mask/ckpt/inpaint_it_200000.pkl')
        self.generator.load_state_dict(state_dict['gen'])

        self.data = {}
        self.title = 'show'
        cv2.namedWindow(self.title)
        with open('/content/face_de_mask/ckpt/expression_mu_std.pkl', 'rb') as f:
            exp = pickle.load(f)
        self.std = exp['std']
        self.std[2] *= 4
        self.std[5] *= 4
